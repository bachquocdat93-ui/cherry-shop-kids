             ...
             <div className="flex flex-col gap-5 flex-1">
                ...
                {productForm.source === 'manual' && (
                   <div className="flex flex-col gap-4">
                     ...
                     <div>
                        <label className="block text-sm font-medium text-gray-700 mb-1">Ảnh sản phẩm (Tùy chọn)</label>
                        <div className="flex items-start gap-4">
                          {productForm.imageUrl ? (
                             <img src={productForm.imageUrl} alt="Preview" className="w-16 h-16 object-cover rounded-md border border-gray-300 shrink-0" />
                          ) : (
                             <div className="w-16 h-16 bg-gray-50 border border-gray-200 border-dashed rounded-md shrink-0 flex flex-col items-center justify-center text-gray-400">
                          ...
                          <div className="flex-1">
                             <input type="file" accept="image/*" onChange={handleImageChange} className="block w-full text-sm text-gray-500 file:mr-4 file:py-2 file:px-4 file:rounded-md file:border-0 file:text-sm file:font-semibold file:bg-primary-50 file:text-primary-700 hover:file:bg-primary-100 transition-colors cursor-pointer" />
                             {productForm.imageUrl && (
                                <button type="button" onClick={() => setProductForm(prev => ({...prev, imageUrl: ''}))} className="mt-2 text-xs text-red-500 hover:text-red-700 font-medium tracking-wide">XÓA ẢNH</button>
                             )}
                          </div>
                        </div>
                     </div>
                    ...
                    <ul className="divide-y divide-gray-100 bg-white border border-gray-200 rounded-lg shadow-sm">
                      {addedItems.map((item, index) => (
                        <li key={item.id} className="p-4 flex items-center justify-between hover:bg-gray-50 transition-colors">
                           <div className="flex items-center gap-3 min-w-0 flex-1">
                              {item.imageUrl ? (
                                  <img src={item.imageUrl} alt="" className="w-10 h-10 object-cover rounded border border-gray-200 shrink-0" />
                              ) : (
                      ...
                      {productForm.productName && productForm.retailPrice >= 0 && productForm.quantity > 0 && (
                          <li className="p-4 flex items-center justify-between bg-gray-50 opacity-60">
                             <div className="flex items-center gap-3 min-w-0 flex-1">
                                {productForm.imageUrl ? (
                                    <img src={productForm.imageUrl} alt="" className="w-10 h-10 object-cover rounded border border-gray-200 shrink-0 grayscale" />
                                ) : (
                                    <div className="w-10 h-10 bg-gray-100 rounded border border-gray-200 flex items-center justify-center shrink-0">
                                        <ShoppingBagIcon className="w-5 h-5 text-gray-400" />
                                    </div>
                             ...
                             <span className="text-sm font-medium text-gray-500 shrink-0">+{new Intl.NumberFormat('vi-VN').format(productForm.retailPrice * productForm.quantity)}đ</span>
                          </li>
                      )}
                    </ul>
                 )}
