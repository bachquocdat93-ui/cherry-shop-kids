               ...
               <div className="space-y-3 relative before:absolute before:inset-y-0 before:-left-3 before:w-px before:bg-slate-200 ml-3">
                 ...
                 {addedItems.map((added, index) => (
                    <div key={added.id} className="flex justify-between items-center group bg-white p-3.5 rounded-2xl border border-slate-200 shadow-sm hover:shadow-md hover:border-slate-300 transition-all relative overflow-hidden ml-3">
                      ...
                      <div className="pl-2">
                        <p className="text-xs font-black text-slate-800">{index + 1}. {added.productName}</p>
                        <p className="text-[11px] font-bold text-slate-500 mt-1">{added.quantity} x <span className="text-blue-600">{new Intl.NumberFormat('vi-VN', { style: 'currency', currency: 'VND' }).format(added.consignmentPrice)}</span></p>
                      </div>
            ...
            <div className="p-5 border border-slate-200 bg-white rounded-2xl relative shadow-sm hover:shadow-md transition-shadow">
                ...
                <div className="space-y-5">
                    <div>
                      ...
                      <input type="text" id="productName" name="productName" value={productForm.productName} onChange={handleChange} className="block w-full rounded-xl border-slate-200 shadow-sm focus:border-primary focus:ring-4 focus:ring-primary/10 text-sm font-bold bg-slate-50/30 focus:bg-white py-2.5 px-3 transition-colors" placeholder="Váy hoa cúc..." />
                    </div>

                    <div className="grid grid-cols-2 gap-4 sm:grid-cols-3">
                      <div className="col-span-1">
                        <label htmlFor="consignmentPrice" className="block text-[10px] font-black text-slate-400 uppercase tracking-widest mb-1.5">Giá Bán <span className="text-red-500">*</span></label>
                        <input type="number" id="consignmentPrice" name="consignmentPrice" value={productForm.consignmentPrice} onChange={handleChange} required min="0" className="block w-full rounded-xl border-slate-200 shadow-sm focus:border-primary focus:ring-4 focus:ring-primary/10 text-sm font-bold bg-slate-50/30 focus:bg-white py-2.5 px-3 transition-colors" />
                      </div>
                      <div className="col-span-1">
